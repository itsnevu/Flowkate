<div align="center">

# Flowkite

**Tarayıcınızda yerel olarak planlayıp harekete geçen yapay zekâ tarayıcı ajanı.**

[![Website](https://img.shields.io/badge/Website-flowkite.xyz-000000?style=for-the-badge&logo=googlechrome&logoColor=white)](https://flowkite.xyz)
[![Chrome Web Store](https://img.shields.io/badge/Install-Chrome_Web_Store-000000?style=for-the-badge&logo=googlechrome&logoColor=white)](https://chromewebstore.google.com/detail/flowkite-ai-web-agent-aut/kacmblccfhnfcjfpeageldmmbkocafab)

[English](README.md) · [Español](README-es.md) · **Türkçe** · [繁體中文](README-zh-Hant.md) · [Português do Brasil](README-pt-BR.md)

</div>

---

## Flowkite’ı edinin

Flowkite, tarayıcınızda yerel olarak planlayıp harekete geçen, indirmesi ücretsiz bir yapay zekâ web ajanıdır — ve önemli bir şey yapmadan önce size sorar.

### Kurulum

**Önerilen (tek tıkla):** [Flowkite’ı Chrome Web Mağazası’ndan edinin →](https://chromewebstore.google.com/detail/flowkite-ai-web-agent-aut/kacmblccfhnfcjfpeageldmmbkocafab)

**Ya da yerel derlemeyi indirin:** `flowkite.zip` dosyasını indirmek ve sitedeki yerel kurulum rehberini izlemek için [flowkite.xyz](https://flowkite.xyz) adresini ziyaret edin.

---

## Bağlantılar

- Web sitesi: [flowkite.xyz](https://flowkite.xyz)
- Chrome Web Mağazası: [Flowkite’ı yükleyin](https://chromewebstore.google.com/detail/flowkite-ai-web-agent-aut/kacmblccfhnfcjfpeageldmmbkocafab)

<div align="center">

[itsnevu](https://github.com/itsnevu) tarafından geliştirildi

</div>
