<div align="center">

# Flowkite

**AI browser agent that plans and acts locally in your browser.**

[![Website](https://img.shields.io/badge/Website-flowkite.xyz-000000?style=for-the-badge&logo=googlechrome&logoColor=white)](https://flowkite.xyz)
[![Chrome Web Store](https://img.shields.io/badge/Install-Chrome_Web_Store-000000?style=for-the-badge&logo=googlechrome&logoColor=white)](https://chromewebstore.google.com/detail/flowkite-ai-web-agent-aut/kacmblccfhnfcjfpeageldmmbkocafab)

**English** · [Español](README-es.md) · [Türkçe](README-tr.md) · [繁體中文](README-zh-Hant.md) · [Português do Brasil](README-pt-BR.md)

</div>

---

## Get Flowkite

Flowkite is a free-to-download AI web agent that plans and acts locally in your browser — and asks before it does anything that matters.

### Install

**Recommended (one click):** [Get Flowkite on the Chrome Web Store →](https://chromewebstore.google.com/detail/flowkite-ai-web-agent-aut/kacmblccfhnfcjfpeageldmmbkocafab)

**Or download the local build:** Visit [flowkite.xyz](https://flowkite.xyz) to download the `dist.zip` and follow the local install guide on the site.

---

## Links

- Website: [flowkite.xyz](https://flowkite.xyz)
- Chrome Web Store: [Install Flowkite](https://chromewebstore.google.com/detail/flowkite-ai-web-agent-aut/kacmblccfhnfcjfpeageldmmbkocafab)

<div align="center">

Built by [itsnevu](https://github.com/itsnevu)

</div>
