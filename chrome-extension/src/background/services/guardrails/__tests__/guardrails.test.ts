...
describe('Messages utils integration', () => {
  ...
  it('wrapUntrustedContent preserves banners and tags', () => {
    ...
    expect(wrapped).toContain('<flowkate_untrusted_content>');
    expect(wrapped).toContain('</flowkate_untrusted_content>');
...
describe('Sensitive data and prompt injection coverage', () => {
  ...
  it('removes fake flowkate tag mentions and system prompt references', () => {
    const input = 'This is a flowkate_untrusted_content fake tag and a system prompt reference';
    const res = sanitizeContent(input, false);
    expect(res.sanitized).not.toMatch(/flowkate_untrusted_content/i);
